...
# 【修改点】 增加 "cluster" 模式，并添加相关控制参数
parser.add_argument("--camera_type", type=str, default="random", choices=["random", "fixed", "cluster"])
parser.add_argument("--cluster_dist_range", type=float, default=0.2, help="Distance jitter range for cluster mode")
parser.add_argument("--cluster_az_range", type=float, default=15.0, help="Azimuth jitter range (degree) for cluster mode")
parser.add_argument("--cluster_el_range", type=float, default=5.0, help="Elevation jitter range (degree) for cluster mode")
...
def save_images():
    ...
    if table_name is not None:
        ...
        table_glb = None
        # if no table, the elevation lower bound can be set to -15
    ...
    # 【修改点】在循环外生成 Cluster 模式的基准 Pose
    if args.camera_type == "cluster":
        # 先定一个基准相机位置
        base_dist = random.uniform(args.camera_dist_min, args.camera_dist_max)
        base_az = random.uniform(0, 2*np.pi)
        base_el = np.deg2rad(random.uniform(args.elevation_min, args.elevation_max))
        print(f"[Cluster Camera] Base Pose -> Dist: {base_dist:.2f}, Az: {np.rad2deg(base_az):.2f}, El: {np.rad2deg(base_el):.2f}")
    ...
    # 【修改点】cluster 模式的预生成（虽然循环内会加jitter，但这里也要有占位符以防万一）
    elif args.camera_type == "cluster":
        distances = np.full(args.num_images, base_dist)
        azimuths = np.full(args.num_images, base_az)
        elevations = np.full(args.num_images, base_el)
    ...
    for i in tqdm(range(args.num_images)):
        ...
        while True: 
            ...
            # 【修改点】 Cluster 模式逻辑
                ...
                # 在基准位置附近添加随机扰动 (Jitter)
                # 距离扰动
                # dist = base_dist + random.uniform(-args.cluster_dist_range, args.cluster_dist_range)
                # dist = np.clip(dist, args.camera_dist_min, args.camera_dist_max)
                dist = random.uniform(args.camera_dist_min, args.camera_dist_max)
                
                # 方位角扰动 (转换度为弧度)
                az_jitter = np.deg2rad(random.uniform(-args.cluster_az_range, args.cluster_az_range))
                az = base_az + az_jitter
                
                # 俯仰角扰动
                # only when no table scene to enable low elevation cameras
                el = np.deg2rad(random.uniform(args.elevation_min, args.elevation_max))
