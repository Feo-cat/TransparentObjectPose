...
def save_images():
    ...
    while True:
        ...
        # 2. 【修改点】相机设置 (Camera Setup - Fixed)
        # 每一轮尝试重新随机一个固定的相机位置
        if args.camera_type == "random":
            # 兼容逻辑，虽然后面循环里不会变
            fix_dist = random.uniform(args.camera_dist_min, args.camera_dist_max)
            fix_az = random.uniform(0, 2*np.pi)
            fix_el = np.deg2rad(random.uniform(args.elevation_min, args.elevation_max))
        else: 
            # Fixed or Continuous (Camera is fixed in position)
            fix_dist = random.uniform(args.camera_dist_min, args.camera_dist_max)
            fix_az = random.uniform(0, 2*np.pi)
            fix_el = np.deg2rad(random.uniform(args.elevation_min, args.elevation_max))
            
        # 设置相机 (这在整个序列中是不变的)
        cam_pt = az_el_to_points(np.array([fix_az]), np.array([fix_el])) * fix_dist
        cam_pt = cam_pt[0]
        scene.frame_set(1)
        camera = set_camera_location(cam_pt)
        bpy.context.view_layer.update()
        
        # 3. 【修改点】物体轨迹生成 (Object Trajectory Generation)
        # 为每个物体生成起点和终点，确保起点和终点都在桌子上且无碰撞
        
        object_motion_plans = []
        
        # 临时辅助函数：寻找有效位置
        def find_valid_pose_for_obj(obj_dat, existing_bboxes):
            root = obj_dat['root']
            ...
            for _ in range(100):
                if table_name is None:
                    # 无桌子
                    if is_liquid:
                        rot_euler = (0, 0, random.uniform(0, 2 * math.pi))
                    else:
                        rot_euler = (
                    ...
                    loc = (random.uniform(-radius, radius), random.uniform(-radius, radius), random.uniform(-radius, radius))
                else:
                    # 有桌子
                    ...
                    if is_liquid:
                        ...
                        if random.random() < 0.5:
                            ...
                            rot_euler = (math.pi / 2, 0, random.uniform(0, 2 * math.pi))
                ...
                if meshes:
                    if table_name is not None:
                        bbox_min, _ = get_world_bbox_from_meshes(meshes)
                    ...
                    if table_name is None:
                        ...
                        if not any(bbox_overlap_3d(bbox3d, b) for b in existing_bboxes):
                            existing_bboxes.append(bbox3d)
                            return root.location.copy(), root.rotation_euler.copy()
                    else:
                        bbox2d = ((bbox_min.x, bbox_min.y), (bbox_max.x, bbox_max.y))
                        if not any(bbox_overlap_2d(bbox2d, b) for b in existing_bboxes):
                            existing_bboxes.append(bbox2d)
                            return root.location.copy(), root.rotation_euler.copy()
                else:
                    return root.location.copy(), root.rotation_euler.copy()
        ...
        for obj_dat in loaded_scene_objects_data:
            s_loc, s_rot = find_valid_pose_for_obj(obj_dat, placed_bboxes_start)
            if s_loc is None: valid_plan = False; break
            
            # 生成终点 (为了简单，终点碰撞检测独立，不考虑起点物体。这在物体移动时可能穿模，但在简单随机场景可接受)
            # 更严格的做法是：终点也必须相对于其他物体的终点无碰撞
            # 这里简化为：生成一个相对较近的随机偏移作为终点，或者重新随机生成一个点
            
            # 策略：重新随机生成一个终点
            # 注意：这里的碰撞检测是基于“所有物体都在终点时互不碰撞”。
            # 实际上这不能保证移动过程中不碰撞，但对于生成简单数据集够用了。
        ...
        if not valid_plan:
            print("  [Setup Failed] Could not place objects (Start). Retrying sequence...")
            continue # Retry outer while loop

        placed_bboxes_end = []
        for plan in current_plans:
            e_loc, e_rot = find_valid_pose_for_obj(plan["obj_dat"], placed_bboxes_end)
            if e_loc is None: valid_plan = False; break
            
            # 约束旋转范围，让旋转看起来更自然 continuous
            # 我们可以保留 s_rot 的 Z 轴基准，加一个随机偏移
            s_rot = plan["start"][1]
            e_rot_constrained = list(e_rot)
            # 如果是液体，只允许 Z 轴旋转
            if plan["obj_dat"]["is_liquid"]:
                 e_rot_constrained[0] = s_rot[0]
                 e_rot_constrained[1] = s_rot[1]
                 # Z 轴旋转限制在 +/- 120 度以内，避免疯转
                 e_rot_constrained[2] = s_rot[2] + random.uniform(-math.pi/1.5, math.pi/1.5)
            else:
                 # 其它物体，也可以做类似限制，这里为了简单直接用随机生成的 end pose
                 pass
            
            plan["end"] = (e_loc, Vector(e_rot_constrained))

        if not valid_plan:
            print("  [Setup Failed] Could not place objects (End). Retrying sequence...")
            continue

        object_motion_plans = current_plans
        ...
        for i in tqdm(range(args.num_images), desc=f"Attempt {sequence_attempt_count}"):
            ...
            # 1. 计算插值并移动物体
            t = i / max(1, args.num_images - 1) # 0.0 to 1.0
            
            for plan in object_motion_plans:
                root = plan["obj_dat"]["root"]
                s_loc, s_rot = plan["start"]
                e_loc, e_rot = plan["end"]
                
                # 线性插值位置
                cur_loc = s_loc.lerp(e_loc, t)
                root.location = cur_loc
                
                # 线性插值旋转 (Euler)
                # 注意：Euler 直接插值在角度跨越 PI/-PI 时可能会有问题，但对于简单随机数据通常可接受
                cur_rot = Vector((0.0, 0.0, 0.0))
                cur_rot[0] = s_rot[0] * (1-t) + e_rot[0] * t
                cur_rot[1] = s_rot[1] * (1-t) + e_rot[1] * t
                cur_rot[2] = s_rot[2] * (1-t) + e_rot[2] * t
                root.rotation_euler = cur_rot
                
            bpy.context.view_layer.update()

            # 2. 重新计算相对位姿矩阵 (物体动了，world2obj 变了)
            # 注意：如果有多物体，这里只取 fixed_obj_names 对应的那个作为 ground truth
            ...
            if whole_obj_visib_ratio < whole_obj_visib_ratio_threshold:
                ...
                valid_azimuths.append(fix_az)
                valid_elevations.append(fix_el)
                valid_distances.append(fix_dist)
