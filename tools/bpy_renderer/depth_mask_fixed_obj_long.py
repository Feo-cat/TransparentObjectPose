...
parser.add_argument("--object_pose_attempts", type=int, default=120)
parser.add_argument("--camera_path_segments_min", type=int, default=10)
parser.add_argument("--camera_path_segments_max", type=int, default=24)
parser.add_argument("--camera_path_segment_len_min", type=int, default=12)
parser.add_argument("--camera_path_segment_len_max", type=int, default=18)
parser.add_argument("--camera_path_attempts", type=int, default=120)
parser.add_argument("--camera_path_validation_samples", type=int, default=7)
parser.add_argument("--camera_az_step_deg_min", type=float, default=10.0)
parser.add_argument("--camera_az_step_deg_max", type=float, default=36.0)
parser.add_argument("--camera_el_step_deg_max", type=float, default=8.0)
parser.add_argument("--camera_dist_step_ratio_min", type=float, default=0.02)
parser.add_argument("--camera_dist_step_ratio_max", type=float, default=0.10)
parser.add_argument("--camera_min_xyz_step_ratio", type=float, default=0.10)
...
def serialize_object_poses(object_poses):
    return [
        {
            "loc": vector_to_list(pose["loc"]),
            "rot": vector_to_list(pose["rot"]),
        }
        for pose in object_poses
    ]

def deserialize_object_poses(serialized_poses):
    return [
        {
            "loc": Vector(pose["loc"]),
            "rot": Vector(pose["rot"]),
...
def serialize_camera_plan(camera_plan):
    return {
        "keyframes": [
            {
                "frame": int(kf["frame"]),
            ...
            for kf in camera_plan["keyframes"]
        ]
    }

def deserialize_camera_plan(serialized_camera_plan):
    return {
        "keyframes": [
            {
                "frame": int(kf["frame"]),
            ...
            for kf in serialized_camera_plan["keyframes"]
        ]
    }
...
def plan_static_object_poses(scene_bundle):
    table_name = scene_bundle["table_name"]
    table_bbox_cfg = scene_bundle["table_bbox_cfg"]
    has_table = table_name is not None
    placed_bboxes = []
    planned_object_poses = []

    for obj_dat in scene_bundle["loaded_scene_objects_data"]:
        placed_pose = None
        candidate_bbox = None

        for _ in range(args.object_pose_attempts):
            raw_pose = generate_initial_pose(obj_dat, table_name, table_bbox_cfg)
            finalized_pose, candidate_bbox = try_finalize_pose(
                obj_dat,
                raw_pose,
            ...
            if finalized_pose is None:
                continue
            if is_pose_bbox_collision_free(candidate_bbox, placed_bboxes, has_table):
                placed_pose = finalized_pose
                break

        if placed_pose is None:
            return None

        planned_object_poses.append({
            "loc": placed_pose[0].copy(),
            "rot": placed_pose[1].copy(),
        })
        placed_bboxes.append(candidate_bbox)

    return planned_object_poses

def apply_static_object_poses(scene_bundle, object_poses):
    for obj_dat, pose in zip(scene_bundle["loaded_scene_objects_data"], object_poses):
        apply_pose_to_object(obj_dat, (pose["loc"], pose["rot"]))
    bpy.context.view_layer.update()

def camera_pose_to_point(camera_pose):
    cam_pt = az_el_to_points(np.array([camera_pose["az"]]), np.array([camera_pose["el"]])) * camera_pose["dist"]
    return Vector(cam_pt[0].tolist())

def interpolate_camera_pose(start_pose, end_pose, t):
    eased_t = smoothstep(t)
    return {
        "az": start_pose["az"] + shortest_angle_delta(start_pose["az"], end_pose["az"]) * eased_t,
        "el": start_pose["el"] + (end_pose["el"] - start_pose["el"]) * eased_t,
        "dist": start_pose["dist"] + (end_pose["dist"] - start_pose["dist"]) * eased_t,
    }

def camera_pose_for_frame(camera_plan, frame_idx):
    keyframes = camera_plan["keyframes"]
    if frame_idx <= keyframes[0]["frame"]:
        return {
            "az": keyframes[0]["az"],
            "el": keyframes[0]["el"],
            "dist": keyframes[0]["dist"],
        }
    if frame_idx >= keyframes[-1]["frame"]:
        return {
            "az": keyframes[-1]["az"],
            "el": keyframes[-1]["el"],
            "dist": keyframes[-1]["dist"],
        }
    ...
    for idx in range(len(keyframes) - 1):
        ...
        if start_kf["frame"] <= frame_idx <= end_kf["frame"]:
            local_t = (frame_idx - start_kf["frame"]) / max(1, end_kf["frame"] - start_kf["frame"])
            return interpolate_camera_pose(start_kf, end_kf, local_t)
...
def sample_next_camera_pose(prev_pose, attempt_idx):
    decay = 1.0 - 0.65 * (attempt_idx / max(1, args.camera_path_attempts - 1))
    az_step_min = math.radians(args.camera_az_step_deg_min) * max(0.5, decay)
    az_step_max = math.radians(args.camera_az_step_deg_max) * decay
    az_step = random.uniform(az_step_min, max(az_step_min, az_step_max))
    az_sign = -1.0 if random.random() < 0.5 else 1.0
    new_az = prev_pose["az"] + az_sign * az_step

    el_step_max = math.radians(args.camera_el_step_deg_max) * decay
    new_el = clamp(
        prev_pose["el"] + random.uniform(-el_step_max, el_step_max),
        math.radians(args.elevation_min),
        math.radians(args.elevation_max),
    )

    dist_range = max(1e-6, args.camera_dist_max - args.camera_dist_min)
    dist_step_min = dist_range * args.camera_dist_step_ratio_min * max(0.5, decay)
    dist_step_max = dist_range * args.camera_dist_step_ratio_max * decay
    dist_step = random.uniform(dist_step_min, max(dist_step_min, dist_step_max))
    new_dist = clamp(
        prev_pose["dist"] + random.uniform(-dist_step, dist_step),
...
def plan_camera_trajectory(frame_nodes, table_bbox_cfg):
    start_pose = {
        "az": random.uniform(0, 2 * math.pi),
        "el": math.radians(random.uniform(args.elevation_min, args.elevation_max)),
        "dist": random.uniform(args.camera_dist_min, args.camera_dist_max),
    }

    keyframes = [{"frame": frame_nodes[0], **start_pose}]
    area_scale = get_motion_area_scale(table_bbox_cfg)
    min_xyz_step = area_scale * args.camera_min_xyz_step_ratio
    ...
    for segment_idx in range(1, len(frame_nodes)):
        ...
        next_pose = None
        prev_pt = camera_pose_to_point(prev_pose)

        for attempt_idx in range(args.camera_path_attempts):
            candidate_pose = sample_next_camera_pose(prev_pose, attempt_idx)
            candidate_pt = camera_pose_to_point(candidate_pose)
            xyz_step = (candidate_pt - prev_pt).length
            relax = 1.0 - 0.55 * (attempt_idx / max(1, args.camera_path_attempts - 1))
            effective_min_xyz_step = min_xyz_step * relax
            if xyz_step < effective_min_xyz_step:
                continue
...
def build_validation_frame_indices(plan_with_keyframes, total_frames):
    ...
    keyframes = plan_with_keyframes["keyframes"]
...
def validate_camera_plan(scene_bundle, object_poses, camera_plan, worker_tag="planner"):
    validation_frames = build_validation_frame_indices(camera_plan, args.num_images)
    ...
    apply_static_object_poses(scene_bundle, object_poses)
    ...
    for frame_idx in tqdm(validation_frames, desc="Plan validation"):
        camera_pose = camera_pose_for_frame(camera_plan, frame_idx)
        set_camera_from_pose(camera_pose)
...
def build_scene_plan(out_dir_path):
    ...
    while True:
        ...
        object_poses = plan_static_object_poses(scene_bundle)
        if object_poses is None:
            print("  [Setup Failed] Could not place static objects without collision. Retrying sequence...")
            continue
        ...
        frame_nodes = build_motion_frame_nodes(
            args.num_images,
            args.camera_path_segments_min,
            args.camera_path_segments_max,
            args.camera_path_segment_len_min,
            args.camera_path_segment_len_max,
        )
        print(f"  [Camera] Frame nodes: {frame_nodes}")

        camera_plan = plan_camera_trajectory(frame_nodes, scene_bundle["table_bbox_cfg"])
        if camera_plan is None:
            print("  [Setup Failed] Could not generate long camera trajectory. Retrying sequence...")
            continue

        if validate_camera_plan(scene_bundle, object_poses, camera_plan, worker_tag="planner"):
            plan_dict = {
                "version": 1,
                "num_images": args.num_images,
                ...
                "object_poses": serialize_object_poses(object_poses),
                "camera_plan": serialize_camera_plan(camera_plan),
            }
            return plan_dict

def render_full_frame(scene_bundle, camera, frame_idx, worker_tag, current_camera_pose):
    links = scene_bundle["links"]
    rl_node = scene_bundle["rl_node"]
    composite_node = scene_bundle["composite_node"]
    mask_out_node = scene_bundle["mask_out_node"]
    ...
    np.savez(
        ...
        azimuth=current_camera_pose["az"],
        elevation=current_camera_pose["el"],
        distance=current_camera_pose["dist"],
    )
...
def render_from_plan(plan_dict):
    ...
    object_poses = deserialize_object_poses(plan_dict["object_poses"])
    apply_static_object_poses(scene_bundle, object_poses)
    camera_plan = deserialize_camera_plan(plan_dict["camera_plan"])
    ...
    for frame_idx in tqdm(range(frame_start, frame_end + 1), desc=f"Worker {worker_tag}"):
        current_camera_pose = camera_pose_for_frame(camera_plan, frame_idx)
        camera = set_camera_from_pose(current_camera_pose)
        render_full_frame(scene_bundle, camera, frame_idx, worker_tag, current_camera_pose)
