        ...
        for scene_id in tqdm(sel_scene_ids, postfix=f"{split}_{scene}"):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    bgr_gl = Image.open(osp.join(data_root, f"{scene_id:06d}/rgb/{int_im_id:06d}.png"))
                    bgr_gl = np.array(bgr_gl)
                    depth_gl = np.load(osp.join(data_root, f"{scene_id:06d}/depth/{int_im_id:06d}.npy"))
                    depth_gl = depth_gl.astype("float32")
                    if depth_gl.ndim == 3:
                        depth_gl = depth_gl[:, :, 0]
                    obj_mask = Image.open(osp.join(data_root, f"{scene_id:06d}/mask/{int_im_id:06d}_{anno_i:06d}.png"))
                    ...
                    if mask.sum() == 0:  # NOTE: this should be ignored at training phase
                        ...
                        if VIS:
                            ...
                            grid_show(show_ims, show_titles, row=1, col=3, save_path=f"/home/renchengwei/GDR-Net/debug/xyz_crop_{scene_id:06d}_{int_im_id:06d}_{anno_i:06d}.png", show=False)
