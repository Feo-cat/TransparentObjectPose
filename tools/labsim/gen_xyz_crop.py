...
data_dir = osp.normpath(osp.join(PROJ_ROOT, f"datasets/BOP_DATASETS/labsim/{data_split}"))

cls_indexes = sorted(idx2class.keys())
cls_names = [idx2class[cls_idx] for cls_idx in cls_indexes]
lm_model_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/labsim/models"))
model_paths = [osp.join(lm_model_dir, f"obj_{obj_id:06d}.ply") for obj_id in cls_indexes]
texture_paths = None

# scenes = [i for i in range(0, 49 + 1)]
# scenes = [1]
scenes = [i+1 for i in range(len(idx2class))]
xyz_root = osp.normpath(osp.join(PROJ_ROOT, f"datasets/BOP_DATASETS/labsim/{data_split}/xyz_crop"))
if not osp.exists(xyz_root):
    os.makedirs(xyz_root)
    ...
    def main(self):
        ...
        for scene_id in tqdm(sel_scene_ids, postfix=f"{split}_{scene}"):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                folder_idx, file_idx = str_im_id.split("_")
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    os.makedirs(osp.join(xyz_root, f"{scene_id:06d}/{folder_idx:06d}"), exist_ok=True)
                    save_path = osp.join(
                        xyz_root,
                        f"{scene_id:06d}/{folder_idx:06d}/{file_idx:06d}_{anno_i:06d}.pkl",
                    )
                    ...
                    bgr_gl = Image.open(osp.join(data_root, f"{scene_id:06d}/rgb/{folder_idx:06d}/{file_idx:06d}.png"))
                    bgr_gl = np.array(bgr_gl)
                    depth_gl = np.load(osp.join(data_root, f"{scene_id:06d}/depth/{folder_idx:06d}/{file_idx:06d}.npy"))
                    depth_gl = depth_gl.astype("float32")
                    if depth_gl.ndim == 3:
                        depth_gl = depth_gl[:, :, 0]
                    obj_mask = Image.open(osp.join(data_root, f"{scene_id:06d}/mask_visib/{folder_idx:06d}/{file_idx:06d}_{anno_i:06d}.png"))
                    obj_mask = np.array(obj_mask)
                    obj_mask = obj_mask > 0
                    # mask = (depth_gl > 0).astype("uint8") & obj_mask
                    mask = obj_mask

                    if mask.sum() == 0:  # NOTE: this should be ignored at training phase
                        print(
                            f"not visible, split {split} scene {scene_id}, im {folder_idx:06d}_{file_idx:06d}_{anno_i:06d} obj {idx2class[obj_id]} {obj_id}"
                        )
                        print(f"{save_path}")
                        xyz_info = {
                        ...
                        if VIS:
                            im_path = osp.join(
                                data_root,
                                f"{scene_id:06d}/rgb/{folder_idx:06d}/{file_idx:06d}.jpg",
                            )
                            im = mmcv.imread(im_path)

                            mask_path = osp.join(
                                data_root,
                                f"{scene_id:06d}/mask/{folder_idx:06d}/{file_idx:06d}_{anno_i:06d}.png",
                            )
                            mask_visib_path = osp.join(
                                data_root,
                                f"{scene_id:06d}/mask_visib/{folder_idx:06d}/{file_idx:06d}_{anno_i:06d}.png",
                            )
                            ...
                            raise RuntimeError(f"split {split} scene {scene_id}, im {folder_idx:06d}_{file_idx:06d}_{anno_i:06d}")
                    else:
                        ...
                        xyz_crop = xyz_crop * crop_mask[..., None]
                        ...
                        if VIS:
                            ...
                            grid_show(show_ims, show_titles, row=1, col=3, save_path=f"/home/renchengwei/GDR-Net/debug/xyz_crop_{scene_id:06d}_{folder_idx:06d}_{file_idx:06d}_{anno_i:06d}.png", show=False)
