...
DATASETS_ROOT = osp.normpath(os.environ.get("GDRN_DATASETS_ROOT", osp.join(PROJ_ROOT, "datasets")))
...
class LM_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.scene_frame_num = data_cfg.get("scene_frame_num", 12)
        ##################################################

        # NOTE: careful! Only the selected objects
        if self.name == "labsim_train" or self.name == "labsim_test":
            self.cat_ids = [cat_id for cat_id, obj_name in ref.labsim_full.id2obj.items() if obj_name in self.objs]
        else:
    ...
    def __call__(self):  # LM_Dataset
        ...
        print(f"Ann files: {self.ann_files}")
        for ann_file, scene_root, xyz_root in zip(tqdm(self.ann_files), self.image_prefixes, self.xyz_prefixes):
            ...
            for im_id in tqdm(indices):
                ...
                for i in range(self.scene_frame_num):
                    ...
                    K = np.array(cam_dict[curr_im_id]["cam_K"], dtype=np.float32).reshape(3, 3)
                    depth_factor = 1000.0 / cam_dict[curr_im_id]["depth_scale"]
                    ...
                    insts = []
                    for anno_i, anno in enumerate(gt_dict[curr_im_id]):
                        obj_id = anno["obj_id"]
                        ...
                        if self.name == "labsim_train" or self.name == "labsim_test":
                            ...
                            raise ValueError(f"Name is not labsim_train or labsim_test: {self.name}")
                        ...
                        bbox_visib = gt_info_dict[curr_im_id][anno_i]["bbox_visib"]
                        bbox_obj = gt_info_dict[curr_im_id][anno_i]["bbox_obj"]
                        ...
                        # if lm
                        if not (self.name == "lm_13_train" or self.name == "lm_13_test"):
                            mask_single = mask_single / 255.0
                ...
                dataset_dicts.append(record_list)
        ...
        for obj_name in self.objs:
            if self.name == "labsim_train" or self.name == "labsim_test":
                model = inout.load_ply(
                    osp.join(self.models_root, f"obj_{ref.labsim_full.obj2id[obj_name]:06d}.ply"),
...
LM_13_OBJECTS = []  # no bowl, cup
LM_OCC_OBJECTS = []

LM_COPY_OBJECTS = []  # no bowl, cup
################################################################################
...
SPLITS_LM = dict(
    ...
    lm_copy_train=dict(
        name="lm_copy_train",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lm_copy/"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lm_copy/models"),
        objs=LM_COPY_OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm_copy/image_set/{}_{}.txt".format(_obj, "train"))
            for _obj in LM_COPY_OBJECTS
        ],
        image_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm_copy/test/{:06d}".format(ref.lm_full.obj2id[_obj]))
            for _obj in LM_COPY_OBJECTS
        ],
        xyz_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm_copy/test/xyz_crop/{:06d}".format(ref.lm_full.obj2id[_obj]))
            for _obj in LM_COPY_OBJECTS
        ],
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
    ...
    lm_copy_test=dict(
        ...
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lm_copy/"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lm_copy/models"),
        objs=LM_COPY_OBJECTS,
        ann_files=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm_copy/image_set/{}_{}.txt".format(_obj, "test"))
            for _obj in LM_COPY_OBJECTS
        ],
        # NOTE: scene root
        image_prefixes=[
    ...
    labsim_train=dict(
        name="labsim_train",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/models"),
        objs=LABSIM_OBJECTS,  # selected objects
        ann_files=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/image_set/{}_{}.txt".format(_obj, "train"))
            for _obj in LABSIM_OBJECTS
        ],
        image_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/train/{:06d}".format(ref.labsim_full.obj2id[_obj]))
            for _obj in LABSIM_OBJECTS
        ],
        xyz_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/train/xyz_crop/{:06d}".format(ref.labsim_full.obj2id[_obj]))
            for _obj in LABSIM_OBJECTS
        ],
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
    ...
    labsim_test=dict(
        ...
        objs=LABSIM_OBJECTS,
        ann_files=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/image_set/{}_{}.txt".format(_obj, "test"))
            for _obj in LABSIM_OBJECTS
        ],
        # NOTE: scene root
        image_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/test/{:06d}").format(ref.labsim_full.obj2id[_obj])
            for _obj in LABSIM_OBJECTS
        ],
        xyz_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/test/xyz_crop/{:06d}".format(ref.labsim_full.obj2id[_obj]))
            for _obj in LABSIM_OBJECTS
        ],
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
...
for obj in ref.labsim_full.objects:
    for split in ["train", "test", "all"]:
        name = "labsim_{}_{}".format(obj, split)
        ann_files = [osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/image_set/{}_{}.txt".format(obj, split))]
        ...
        if name not in SPLITS_LM:
            SPLITS_LM[name] = dict(
                ...
                ann_files=ann_files,
                image_prefixes=[osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/test/{:06d}").format(ref.labsim_full.obj2id[obj])],
                xyz_prefixes=[
                    osp.join(DATASETS_ROOT, "BOP_DATASETS/labsim/test/xyz_crop/{:06d}".format(ref.labsim_full.obj2id[obj]))
                ],
                scale_to_meter=0.001,
                with_masks=True,  # (load masks but may not use it)
                with_depth=True,  # (load depth path here, but may not use it)
