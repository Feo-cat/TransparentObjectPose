...
def gdrn_inference_on_dataset(cfg, model, data_loader, evaluator, amp_test=False):
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            #############################
            # process input — data is already in (B, N, ...) format
            if not isinstance(inputs, list):  # bs=1
                inputs = [inputs]
            sample = inputs[0]
            device = "cuda" if torch.cuda.is_available() else "cpu"

            # Data shape: (B, N, ...) where B=num_inst, N=N_views
            num_inst = sample["roi_img"].shape[0]  # B
            N_views = sample["roi_img"].shape[1]   # N
            target_mode = cfg.TEST.get("TARGET_IDX_MODE", "last")
            if target_mode == "last":
                target_idx = -1
            elif target_mode == "all_except_first":
                target_idx = list(range(1, N_views))
                if len(target_idx) == 0:
                    target_idx = [0]
            elif target_mode == "all":
                target_idx = list(range(N_views))
            elif target_mode == "random_train_like":
                rand_num = np.random.randint(1, N_views + 1)
                start_idx = np.random.randint(0, N_views - rand_num + 1)
                selected_window = list(range(start_idx, start_idx + rand_num))
                target_idx = selected_window[1:] if len(selected_window) > 1 else [selected_window[0]]
            else:
                raise ValueError(f"Unknown TEST.TARGET_IDX_MODE: {target_mode}")
            target_idx_eval, has_context_for_target, is_all_target = analyze_target_indices(target_idx, N_views)

            # Check train_objs filter
            if evaluator.train_objs is not None:
                roi_labels = sample["roi_cls"].cpu().numpy().tolist()
                obj_names = [evaluator.obj_names[_l] for _l in roi_labels]
                if all(_obj not in evaluator.train_objs for _obj in obj_names):
                    continue

            # Data is already (B, N, ...) — send to device directly
            ...
            roi_wh = sample["roi_wh"].to(device)                # (B, N, 2)
            roi_center = sample["bbox_center"].to(device)       # (B, N, 2)
            resize_ratio = sample["resize_ratio"].to(device)    # (B, N)
            roi_coord_2d = sample["roi_coord_2d"].to(device) if "roi_coord_2d" in sample else None
            roi_extent = sample["roi_extent"].to(device) if "roi_extent" in sample else None
            input_images = sample["input_images"].to(device)    # (B, N, C, H, W)
            patch_mask = sample["patch_mask"].to(device)        # (B, N, H//16, W//16)
            scale = sample["scale"].to(device) if "scale" in sample else None  # (B, N, 1)
            vi_model_kwargs, vi_missing = collect_view_interaction_model_kwargs(sample, device=device)

            vi_cfg = cfg.MODEL.CDPN.get("VIEW_INTERACTION", {})
            if (
                vi_cfg.get("ENABLED", False)
                and (has_context_for_target or (is_all_target and N_views >= 2))
                and len(vi_missing) > 0
            ):
                logger.warning(
                    "VIEW_INTERACTION is enabled but evaluator batch is missing %s; "
                    "evaluation will fall back to the baseline forward for this batch.",
                    ",".join(vi_missing),
                )
            ...
            # Prepare inputs for evaluator — extract target view data
            ...
            score_val = sample.get("score", None)
            if score_val is not None and isinstance(score_val, torch.Tensor):
                if score_val.dim() > 1:
                    score_val = score_val[:, target_idx]
                    if is_list_target:
                        score_val = score_val.reshape(num_inst * num_target_views)
            else:
                score_val = torch.ones(num_inst * num_target_views if is_list_target else num_inst)

            if is_list_target:
                target_input = {
                    "roi_img": sample["roi_img"][:, target_idx].reshape(num_inst * num_target_views, *sample["roi_img"].shape[2:]),
                    "roi_cls": sample["roi_cls"].repeat_interleave(num_target_views, dim=0),
                    "cam": sample["cam"][:, target_idx].reshape(num_inst * num_target_views, *sample["cam"].shape[2:]),
                    "score": score_val,
                    "roi_coord_2d": (
                        sample["roi_coord_2d"][:, target_idx].reshape(num_inst * num_target_views, *sample["roi_coord_2d"].shape[2:])
                        if "roi_coord_2d" in sample else None
                    ),
                    "bbox_center": sample["bbox_center"][:, target_idx].reshape(num_inst * num_target_views, *sample["bbox_center"].shape[2:]),
                    "roi_extent": (
                        sample["roi_extent"][:, target_idx].reshape(num_inst * num_target_views, *sample["roi_extent"].shape[2:])
                        if "roi_extent" in sample else None
                    ),
                    "scale": (
                        sample["scale"][:, target_idx].reshape(num_inst * num_target_views, *sample["scale"].shape[2:])
                        if "scale" in sample else None
                    ),
                    "resize_ratio": (
                        sample["resize_ratio"][:, target_idx].reshape(num_inst * num_target_views)
                        if "resize_ratio" in sample else None
                    ),
                }
            else:
                target_input = {
                    "roi_img": sample["roi_img"][:, target_idx],   # (B, C, H, W)
                    "roi_cls": sample["roi_cls"],                   # (B,)
                    "cam": sample["cam"][:, target_idx],            # (B, 3, 3)
                    "score": score_val,                             # (B,)
                    "roi_coord_2d": sample["roi_coord_2d"][:, target_idx] if "roi_coord_2d" in sample else None,
                    "bbox_center": sample["bbox_center"][:, target_idx],  # (B, 2)
                    "roi_extent": sample["roi_extent"][:, target_idx] if "roi_extent" in sample else None,
                    "scale": sample["scale"][:, target_idx] if "scale" in sample else None,  # (B, 1)
                    "resize_ratio": sample["resize_ratio"][:, target_idx] if "resize_ratio" in sample else None,  # (B,)
                }
            # List-type fields: extract target view per instance
            for key in ["file_name", "scene_im_id", "model_info"]:
                if key in sample:
                    val = sample[key]
                    if isinstance(val, list) and len(val) > 0 and isinstance(val[0], list):
                        if is_list_target:
                            target_input[key] = [val[b][v] for b in range(num_inst) for v in target_idx_list]
                        else:
                            # val = [[v0, v1, ...], [v0, v1, ...], ...] per instance
                            target_input[key] = [val[b][target_idx] for b in range(num_inst)]
                    else:
                        target_input[key] = val
            # height, width: list of lists → extract target view
            for key in ["height", "width"]:
                if key in sample:
                    ...
                    if isinstance(val, list) and len(val) > 0 and isinstance(val[0], list):
                        ...
                        target_input[key] = val
            # Map height/width → im_H/im_W for evaluator compatibility
            if "height" in target_input:
                target_input["im_H"] = target_input["height"]
            if "width" in target_input:
                target_input["im_W"] = target_input["width"]

            eval_inputs = [target_input]
            outputs = [{"time": cur_compute_time}]

            start_process_time = time.perf_counter()
            evaluator.process(eval_inputs, outputs, out_dict)  # RANSAC/PnP
