...
class GDRN_EvaluatorCustom(DatasetEvaluator):
    ...
    def process_net_and_pnp(self, inputs, outputs, out_dict, pnp_type="iter"):
        ...
        for i, (_input, output) in enumerate(zip(inputs, outputs)):
            ...
            for inst_i in range(len(_input["roi_img"])):
                ...
                im_H = int(_input["im_H"][inst_i])
                im_W = int(_input["im_W"][inst_i])
    ...
    def get_gts(self):
        ...
        target_mode = self.cfg.TEST.get("TARGET_IDX_MODE", "last")
        for im_dict in tqdm(dataset_dicts):
            # Handle multi-view data: each entry can be a list of view dicts.
            if isinstance(im_dict, list):
                n_views = len(im_dict)
                if target_mode == "last":
                    eval_view_indices = [n_views - 1]
                elif target_mode == "all_except_first":
                    eval_view_indices = list(range(1, n_views))
                    if len(eval_view_indices) == 0:
                        eval_view_indices = [0]
                elif target_mode == "all":
                    eval_view_indices = list(range(n_views))
                elif target_mode == "random_train_like":
                    raise ValueError(
                        "TEST.TARGET_IDX_MODE='random_train_like' is not supported in custom evaluator "
                        "because GT view selection must be deterministic and aligned with predictions. "
                        "Use 'last', 'all_except_first', or 'all'."
                    )
                else:
                    raise ValueError(f"Unknown TEST.TARGET_IDX_MODE: {target_mode}")
                cur_dicts = [im_dict[i] for i in eval_view_indices]
            else:
                cur_dicts = [im_dict]

            for cur_im_dict in cur_dicts:
                file_name = cur_im_dict["file_name"]
                annos = cur_im_dict["annotations"]
                K = cur_im_dict["cam"]
                for anno in annos:
                    quat = anno["quat"]
                    R = quat2mat(quat)
    ...
    def _eval_predictions(self):
        ...
        for metric_name in metric_names:
            ...
            for obj_name in obj_names:
                ...
                if len(res) > 0:
                    line.append(f"{100 * np.mean(res):.6f}")
            ...
            # average
            if len(obj_names) > 0:
                line.append(f"{100 * np.mean(this_line_res):.6f}")
            big_tab.append(line)
        ...
        for error_name in ["re", "te"]:
            ...
            # mean
            if len(obj_names) > 0:
                line.append(f"{np.mean(this_line_res):.6f}")
            big_tab.append(line)
        ### log big tag
