...
def farthest_point_sampling_np(points, num_samples, init_center=True):
    """Farthest point sampling with C++ impl preferred.

    Args:
        points: (N, 3) numpy array.
        num_samples: number of sampled points.
        init_center: if True, start from point nearest to centroid.
    ...
    n = points.shape[0]
    if n == 0:
        raise ValueError("Cannot run FPS on empty point set.")
    if num_samples >= n:
        return points.astype(np.float32, copy=True)

    pts = points.astype(np.float32, copy=False)

    # Prefer project C++ FPS implementation for speed and consistency.
    try:
        from core.csrc.fps.fps_utils import farthest_point_sampling

        return farthest_point_sampling(pts, num_samples, init_center=init_center)
    except Exception:
        # Fallback to deterministic numpy FPS when extension is unavailable.
        pass
    ...
    if init_center:
        ...
        first_idx = int(np.argmin(np.sum((pts - center) ** 2, axis=1)))
    else:
        first_idx = 0

    selected[0] = first_idx
    min_dists = np.sum((pts - pts[first_idx:first_idx + 1]) ** 2, axis=1)

    for i in range(1, num_samples):
        farthest_idx = int(np.argmax(min_dists))
        selected[i] = farthest_idx
        dists = np.sum((pts - pts[farthest_idx:farthest_idx + 1]) ** 2, axis=1)
