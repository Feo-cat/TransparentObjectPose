...
def depth_to_cam_points(depth, K, valid_mask):
...
def cam_to_obj(points_cam, R, t):
    if points_cam.shape[0] == 0:
        return points_cam
    # pose convention: X_cam = R @ X_obj + t  =>  X_obj = R^T @ (X_cam - t)
    return ((R.T @ (points_cam.T - t.reshape(3, 1))).T).astype(np.float32)
...
def get_emb_show_percentile(bbox_emb, lower_percentile=10, upper_percentile=90):
    show_emb = bbox_emb.copy()
    show_emb_min = np.percentile(show_emb, lower_percentile)
    show_emb_max = np.percentile(show_emb, upper_percentile)
    show_emb = np.clip(show_emb, show_emb_min, show_emb_max)
...
def allo_to_ego_mat_torch(translation, rot_allo, eps=1e-4):
    ...
    # Compute rotation between ray to object centroid and optical center ray
    axis = torch.linalg.cross(cam_ray.expand_as(obj_ray), obj_ray, dim=-1)
    axis = axis / (torch.norm(axis, dim=1, keepdim=True) + eps)
