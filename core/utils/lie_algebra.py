...
def rot_to_lie_vec_direct(R: torch.Tensor) -> torch.Tensor:
    """
    Directly extract Lie vector (axis-angle) from a Rotation Matrix.
    Bypasses quaternions to avoid deeply nested torch.where gradients causing NaN.
    """
    if not isinstance(R, torch.Tensor):
        raise TypeError(f"Input type is not a torch.Tensor. Got {type(R)}")

    # 1. 计算矩阵的迹 Trace
    trace = R[..., 0, 0] + R[..., 1, 1] + R[..., 2, 2]
    
    # 2. 计算旋转角 Theta (加上严格的 clamp 防御 acos 边缘梯度无穷大)
    cos_theta = (trace - 1.0) / 2.0
    cos_theta = torch.clamp(cos_theta, -1.0 + 1e-6, 1.0 - 1e-6)
    theta = torch.acos(cos_theta)
    
    # 3. 提取非归一化的旋转轴向量 (v = R - R^T)
    v0 = R[..., 2, 1] - R[..., 1, 2]
    v1 = R[..., 0, 2] - R[..., 2, 0]
    v2 = R[..., 1, 0] - R[..., 0, 1]
    v = torch.stack([v0, v1, v2], dim=-1)
    
    # 4. 计算缩放系数 coeff = theta / (2 * sin(theta))
    # 为防止 sin(theta) = 0 (即 theta=0 恒等变换时) 除以 0，使用泰勒展开
    sin_theta = torch.sqrt(torch.clamp(1.0 - cos_theta * cos_theta, min=1e-10))
    
    coeff_normal = theta / (2.0 * sin_theta)
    
    # 当 theta 趋近于 0 时，theta / (2 * sin(theta)) 的泰勒展开极限为 1/2 + (theta^2)/12
    coeff_taylor = 0.5 + (theta * theta) / 12.0
    
    # 使用安全的掩码平滑过渡
    small_angle_mask = (theta < 1e-3)
    coeff = torch.where(small_angle_mask, coeff_taylor, coeff_normal)
    
    # 5. 组合得到李代数向量
    lie_vec = v * coeff.unsqueeze(-1)
    return lie_vec
...
def quaternion_to_lie_vec(quaternion: torch.Tensor) -> torch.Tensor:
    ...
    # ==========================================================
    # 🌟 修复: 增加极其微小的 Clamp 垫底，彻底杜绝 sqrt(0) 和 0/0
    # ==========================================================
    sin_squared_theta_safe = torch.clamp(sin_squared_theta, min=1e-15)
    sin_theta: torch.Tensor = torch.sqrt(sin_squared_theta_safe)
    # ==========================================================
