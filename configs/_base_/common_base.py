...
INPUT = dict(
    ...
    SMOOTH_XYZ=False,
    ## mask noise augmentation (simulates noisy inference masks)
    MASK_NOISE_AUG_PROB=0.0,          # per-sample probability (0=off)
    MASK_NOISE_BLOB_PROB=0.3,         # random false-positive blobs
    MASK_NOISE_SPUR_PROB=0.2,         # thin connected spurs from boundary
    MASK_NOISE_ERODE_PROB=0.3,        # boundary erosion
    MASK_NOISE_DILATE_PROB=0.3,       # boundary dilation
    MASK_NOISE_DROPOUT_PROB=0.2,      # random rectangular dropout
)
...
TRAIN = dict(
    ...
    VIS_IMG=False,
    VIS_POSE_VIDEO=False,
    VIS_POSE_VIDEO_INTERVAL=200,
...
# ---------------------------------------------------------------------------- #
# Checkpoint sync (e.g. upload to OSS after local save)
# ---------------------------------------------------------------------------- #
CKPT_SYNC = dict(
    ENABLED=False,
    # Example:
    # - ossutil: "oss://your-bucket/path/to/exp"
    # - ads-cli: "s3://AK:SK@endpoint/bucket/path/to/exp"
    REMOTE_DIR="",
    # Available placeholders: {local_path}, {remote_path}, {filename}, {remote_dir}
    CMD_TEMPLATE="ossutil cp -f {local_path} {remote_path}",
    # Remove local checkpoint only when upload succeeds.
